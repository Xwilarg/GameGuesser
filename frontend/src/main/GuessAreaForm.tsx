...
const GuessAreaForm = forwardRef((
    ...
    return <div className="container box is-flex" id="word-list">
    {
        data.map((x, index) => {
            ...
            {
                return <span key={index} className="hidden-word" style={{ // Not found, we just display a black square
                    width: (x.length * 14.4025) + "px"
                }}></span>
            }
            if (x.displayAsClose !== null) {
                return <span key={index} style={{ // We found a similar word, we display it in orange, transparency depending of how close it is
