...
public class ConfigManager(JsonSerializerOptions options, IHttpHandler client, IServiceScopeFactory scopeFactory)
{
    ...
    /// </summary>
    private string[] GetAcceptedWords(string word, string[][] verbs)
    {
            ...
            else // We found some punctuation or a blank space
            {
                if (!string.IsNullOrEmpty(currWord.ToString())) // We stored letters before so we have a pending word
                {
                    ...
                    tokens.Add(new()
                    {
                        ...
                        Word = word,
                        AcceptedWords = GetAcceptedWords(word, verbs)
                    });
                    currWord.Clear();
                }
                tokens.Add(new()
    ...
    private async Task<string> GetSteamApiAnswerAsync(int gameId, Language language)
    {
    ...
    public async Task<int> UpdateAsync(Language language, string now)
    {
        ...
        if (!configWork.IsUpToDate(now))
        {
            ...
            try
            {
                ...
                var steamEnglish = await GetSteamApiAnswerAsync(gameId, Language.English);
                localConfigWork.SetSteamAnswer(Language.English, steamEnglish);
                var steamEnglishDesc = ParseSteamApiAnswer(steamEnglish, steamJsonOpt).Data.ShortDescription;

                foreach (var lang in Enum.GetValues(typeof(Language)).Cast<Language>().Where(x => x != Language.English))
                {
                    var steamVal = await GetSteamApiAnswerAsync(gameId, lang);
                    var steamValDesc = ParseSteamApiAnswer(steamVal, steamJsonOpt).Data.ShortDescription;
                    localConfigWork.SetSteamAnswer(lang, steamEnglishDesc == steamValDesc ? null : steamVal); // If the description is the same one as english, it means the game don't have a local translation
                }
            }
            finally
            {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var verbs = localConfigWork.GetVerbs(language);

                SetProgression(language, 0);

                // Get game data
                Console.WriteLine($"Getting local {language} of game {steamData.Name}");
            ...
            {
                localConfigWork.ToggleUpdateFlag(language, false);
            }
        });
